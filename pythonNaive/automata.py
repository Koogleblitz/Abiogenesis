...
#TODO:--------------
    ...
    # [] big brain snek
    # [] more snek
    # [] put big brain snek in game of life
    # [] big brain snek have children 
    # [] Pararell sneks with GPU

#[:+:] -- for 16:9 1080p: H=9, W=16, scalar=12, cellsize=10
# less ideal Cellsize= 12, scalar9, HW6,16,
cellSize= 10
scalar= 7
H= 9*scalar
W= 16*scalar
...
class Snek:
    def __init__(self):
        ...
        self.xStep= 1
        self.yStep= 0
        self.headSize= (cellSize, cellSize)
...
def tick(canvas, cellMatrix, cellSize, clk, snek,snake):
    ...
    # if (clk%snekLen):
    #     print("Snek length update: "+ str(snekLen))
    # else:
    #     canvas.fill('black')
    if(snake):
        for rect in snek.bodArr:
            pygame.draw.rect(canvas, rgb_lBlue, (rect))
    ...
    for y, x in np.ndindex(gridTrim):
        ...
        rgbState= (
            rgb_midGreen[0]*cellMatrix[y,x],
            rgb_midGreen[1]*cellMatrix[y,x],
            rgb_midGreen[2]*cellMatrix[y,x]    
        )
        if cellMatrix[y,x]:
            ...
            if neighborhood == 3:
                ...
                rgbState= rgb_darkGreen
    # [+]:: -- //process rectangles -----------------------------------------------------//
        cell = pygame.Rect((cellSize*x, cellSize*y),(cellSize-1, cellSize-1))
        if (x%3):
            if(snake):
                for i in range(0,snekLen):
                    pygame.draw.rect(canvas, rgb_lBlue, snek.bodArr[i])
        #     for i in range(0,snekLen):
        #         snekTangle= ((snek.head.x-cellSize*snek.xStep, snek.head.y-cellSize*snek.yStep),(snek.bodSize))
        #         pygame.draw.rect(canvas, rgb_lBlue, (snekTangle))
                # snek.bodArr.append(pygame.Rect((snek.head.x, snek.head.y),(snek.bodSize)))
                # pygame.draw.rect(canvas, rgb_red,snek.bodArr.pop(i))
    # [+]:: -- \\process rectangles ----------------------------------------------------\\
        render(canvas, cellMatrix, cell, rgbState, clk)
    return matrixReloaded, snek
        



def render(canvas,cellMatrix, cell, rgbState,clk):
    pygame.draw.rect(canvas, rgbState, (cell) )
    #pygame.draw.rect(canvas, rgb_lBlue, (snekTangle))

    # for rect in range(0,snekLen):
    #     pygame.draw.rect(canvas, rgb_beige, snekTangle)
...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    print("snakeY.X:  " + str(snek.head.y) +"." + str(snek.head.x))
            ...
            if pygame.mouse.get_pressed()[1]:
                print('middle click:  ' + str(mouseYXRez) )
        ...
        if not paused:
            ...
            cellMatrix = tick(grid, cellMatrix, cellSize,clk, snek, snake)[0]
            # snek= tick(grid, cellMatrix, cellSize,clk, snek)[1]
            # snekTick(grid,cellMatrix, snek)
            
            pygame.display.update()
            clk+=1
            #if(clk%10):print("Cell Calculations: "+ str(clk))
        #---[+]:: \\Update Functions----------------------------//
